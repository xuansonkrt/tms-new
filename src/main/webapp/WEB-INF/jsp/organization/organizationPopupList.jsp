...
<!--   pagesize="10" -->
    <display:table   class="table table-striped table-bordered" name="resultList" 
        ...
        <display:column title="Chọn" class="pointer btnEdit" style="width: 30px; text-align: center;">
          <i class="fa fa-check" onclick="selectOrg('${row.id}','${row.name}')"></i>
        </display:column>
       
        <display:column property="code" title="Mã đơn vị" autolink="true" />
        <display:column property="name" title="Tên đơn vị" />
